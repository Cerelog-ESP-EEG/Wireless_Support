...
// ==========================================
// These MUST match Python_wifi_LSL.py. The host script broadcasts
// "CERELOG_FIND_ME" on UDP 4445 and then opens a TCP socket on 1112.
...
// Set true only while a TCP client is attached. Gates the DRDY interrupt so
// the ADS1299 is not free-running into nowhere before anyone connects.
...
// --- Acquisition control -------------------------------------------------
// The V1 WiFi host firmware holds the ADS1299 in reset-idle (START low) until
// a TCP client attaches, then runs the settle sequence. That timing is what
// makes the wireless stream clean, so it is reproduced here rather than
// free-running the ADC from setup() the way the USB firmware does.
...
void setup() {
  ...
  // No serial handshake over WiFi: Python_wifi_LSL.py sends no timestamp
  // packet. The reference is re-zeroed on each TCP connect instead (see loop),
  // which is what the V1 WiFi host firmware does.
  ...
  WiFi.setSleep(false);   // modem sleep adds latency/jitter to the stream

  Serial.println("\n=== CERELOG V2 WiFi Access Point ===");
  ...
  // left idle (START low) until a TCP client connects; see start_acquisition().
...
void loop() {
  ...
  if (!client || !client.connected()) {
      if (server->hasClient()) {
          ...
          if (newClient) {
              ...
              client = newClient;
              client.setNoDelay(true);   // no Nagle batching on a 37-byte stream
              Serial.println(">>> Client connected - streaming");
              start_acquisition();
          }
      }
  }
  ...
  // ============================================================
  // PART 2: UDP discovery - answers the host script's broadcast
  // ============================================================
  ...
  if (!client || !client.connected()) {
      if (streaming_allowed) {
          Serial.println("Client lost - acquisition stopped");
          stop_acquisition();
          if (client) client.stop();
      }
      yield();
